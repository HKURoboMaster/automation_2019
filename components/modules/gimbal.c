...
int32_t gimbal_cascade_register(struct gimbal *gimbal, const char *name, enum device_can can)
{
  ...
  pid_struct_init(&(gimbal->cascade[YAW_MOTOR_INDEX].outer), 500, 600, 75, 0, 0);
  pid_struct_init(&(gimbal->cascade[YAW_MOTOR_INDEX].inter), 30000, 3000, 220, 0, 0);

  gimbal->mode.bit.pitch_mode = ENCODER_MODE;
  gimbal->ctrl[PITCH_MOTOR_INDEX].convert_feedback = pitch_ecd_input_convert;
  pid_struct_init(&(gimbal->cascade[PITCH_MOTOR_INDEX].outer), 1000, 200, 0, 0, 0);
  pid_struct_init(&(gimbal->cascade[PITCH_MOTOR_INDEX].inter), 30000, 8000, 0, 0, 0);
  gimbal->ctrl[PIICH_ASSIT_INDEX].convert_feedback = pitch_ecd_input_convert;
  pid_struct_init(&(gimbal->cascade[PIICH_ASSIT_INDEX].outer), 1000, 200, 0, 0, 0);
  pid_struct_init(&(gimbal->cascade[PIICH_ASSIT_INDEX].inter), 30000, 8000, 0, 0, 0);
