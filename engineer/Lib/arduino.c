...
int ard_send_msg(Ard_Tx msg)
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c2,ARDUINO_SLAVE,(uint8_t *)buffer,sizeof(uint8_t)*3,1000)==HAL_OK)
		return 1;
	else
		return 0;
}
...
{
	int buffer[3];
	if(HAL_I2C_Master_Receive(&hi2c2,ARDUINO_SLAVE,(uint8_t*)buffer,sizeof(uint8_t)*3,1000)==HAL_OK)
	{
